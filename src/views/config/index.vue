    ...
    <el-col :span="24" class="toolbar" style="padding-bottom: 0;">
      <el-form :inline="true" :model="listQuery" label-position="left">
        ...
        <el-form-item>
          ...
          <el-button icon="el-icon-plus" type="primary" v-if="hasPermission('sys:config:add')" @click="handleCreate">{{$t('common.add')}}</el-button>
          <el-button icon="el-icon-edit" type="primary" v-if="hasPermission('sys:config:update')" @click="handleUpdate">{{$t('common.edit')}}</el-button>
          <el-button icon="el-icon-delete" type="danger" v-if="hasPermission('sys:config:delete')" @click="handleDelete">{{$t('common.delete')}}</el-button>
          <el-button @click="handleEnable" type="primary" v-if="hasPermission('sys:config:enable')"><svg-icon icon-class="enable"/>{{$t('common.enable')}}</el-button>
          <el-button icon="el-icon-fa fa-stop" type="primary" v-if="hasPermission('sys:config:disable')" @click="handleDisable">{{$t('common.disable')}}</el-button>
        </el-form-item>
      </el-form>
    </el-col>
