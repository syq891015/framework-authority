<template>
  <div>
    ...
    <el-table ref="dataTable" :data="list" v-loading="listLoading" :element-loading-text="$t('common.loadingText')" border :fit="true" highlight-current-row stripe @selection-change="handleSelectionChange" @row-click="handleRowClick" tooltip-effect="light">
      ...
      <el-table-column :label="$t('roleMenu.menuId')" prop="menuId" align="center" />
    </el-table>
    ...
    <el-dialog :close-on-click-modal="false" :title="dialogTitle" :visible.sync="dialogFormVisible" class="help-info">
      <el-form :rules="rules" ref="dataForm" :model="temp" label-position="right" label-width="120px" style='width: 90%;'>
        <el-form-item :label="$t('roleMenu.menuId')" prop="menuId">
          <el-input v-model="temp.menuId" clearable></el-input>
          <el-alert title="菜单ID是必填的；长度应小于$column.maxLength个字符" :closable="false" />
        </el-form-item>
      </el-form>
...
<script>
import { fetchList, createRoleMenu, updateRoleMenu, deleteRoleMenu } from '@/api/roleMenu'
...
export default {
  ...
  data () {
    return {
      ...
      rules: {
        menuId: [
          { required: true, message: '菜单ID是必填的', trigger: 'change' }
        ]
      }
    }
  },
  ...
  methods: {
    ...
    handleDelete () {
      ...
      this.$confirm('确定删除吗?', '提示', {
        ...
        deleteRoleMenu(this.multipleSelection[0].roleId).then(response => {
          if (response.code === 0) {
            this.$message.success({
              message: '删除成功!'
            })
